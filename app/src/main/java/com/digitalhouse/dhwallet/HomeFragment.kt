...
class HomeFragment : Fragment(R.layout.fragment_home) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        recyclerHome.adapter = TransactionAdapter(
            listTransactions,
            title = view.context.getString(R.string.transacao),
            showArrow = true,
            { sendToTransaction() },
            { transaction ->
        ...
        var viewPagerMudou = object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(position: Int) {
                if (position == 0) {
                    recyclerHome.adapter = TransactionAdapter(
                        listTransactions,
                        view.context.getString(R.string.transacao),
                        true,
                        {
                            sendToTransaction()
                        },
    ...
    private fun sendToTransaction() {
        val action =
            HomeFragmentDirections.actionHomeFragmentToTransactionFragment("R$ 45,87", "R$ 589")
